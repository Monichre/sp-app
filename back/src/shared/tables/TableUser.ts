...
const NewDocUser = t.intersection([
  DocUserKeySource,
  DocUserRequired,
  DocUserOptional
]);
...
const UpdateDocUser = t.intersection([
  DocUserKeySource,
  DocUserUpdate,
  DocUserOptional
]);
...
export const TableUser = (
  ...
  const getAllUsers = async () => {
    const { Items: users } = await doc.scan({ TableName }).promise();
    return users;
  };
  ...
  const updateUser = async (obj: TUpdateDocUser) => {
    ...
    const UpdateExpression =
      "SET " + nonNullKeys.map(k => `${k} = :${k}`).join(", ");
