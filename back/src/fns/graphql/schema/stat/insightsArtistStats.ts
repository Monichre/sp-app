...
const playtimePerspectiveStats = async (tableStat: TTableStat, uid: string, artistId: string, periodType: PeriodType, periodCurrent: string, periodPrev?: string) => {
  const currentMs = await tableStat.getArtistStat({uid, periodType, periodValue: periodCurrent, artistId})
  const current = hrsMaybeMins(hrsAndMins(currentMs))
  if (!periodPrev) {
    return { current }
  }
  const prevMs = await tableStat.getArtistStat({uid, periodType, periodValue: periodPrev, artistId})
...
const playtimeTimescopeStats = async (tableStat: TTableStat, {timeseriesLabel, timeseriesPeriodType, uid, gid, artistId, endDate, unit, distance, periodType, periodCurrent, periodPrev}: TPlaytimeTimescopeStatsKeys) => ({
  timeseries: await timeSeries(tableStat, timeseriesLabel, { endDate, unit, distance, uid, gid, periodType: timeseriesPeriodType, relationType: 'artist', relationId: artistId}),
  personal: await playtimePerspectiveStats(tableStat, uid, artistId, periodType, periodCurrent, periodPrev),
  group: await playtimePerspectiveStats(tableStat, 'global', artistId, periodType, periodCurrent, periodPrev),
})

const playtimeStats = async (tableStat: TTableStat, uid: string, gid: string, artistId: string, now: moment.Moment) => {
  ...
  return {
    artist: await tableStat.getArtistInfo(artistId),
    today: await playtimeTimescopeStats(tableStat, {uid, gid, artistId, periodType: 'day', periodCurrent: today, endDate: now, periodPrev: yest, timeseriesLabel: 'past 7 days', timeseriesPeriodType: 'day', unit: 'days', distance: 7}),
    thisWeek: await playtimeTimescopeStats(tableStat, {uid, gid, artistId, periodType: 'week', periodCurrent: thisWeek, endDate: now, periodPrev: lastWeek, timeseriesLabel: 'past 6 weeks', timeseriesPeriodType: 'week', unit: 'weeks', distance: 6}),
    thisMonth: await playtimeTimescopeStats(tableStat, {uid, gid, artistId, periodType: 'month', periodCurrent: thisMonth, endDate: now, periodPrev: lastMonth, timeseriesLabel: 'past 5 months', timeseriesPeriodType: 'month', unit: 'months', distance: 5}),
    lifetime: await playtimeTimescopeStats(tableStat, {uid, gid, artistId, periodType: 'life', periodCurrent: life, endDate: now, timeseriesLabel: 'past 5 months', timeseriesPeriodType: 'month', unit: 'months', distance: 5}),
...
const insightsArtistStats: QueryResolvers.InsightsArtistStatsResolver = async (_, {uid, gid, artistId}, context) => {
  ...
  return await playtimeStats(tableStat, uid, gid, artistId, now)
}
...
export const insightsArtistStatsSchema = makeExecutableSchema({typeDefs, resolvers})