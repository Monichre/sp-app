...
export const determineAchievementValueFromPK = ({ pk }: any) =>
  pk.includes("#first") ? "first" : pk.includes("#second") ? "second" : "third";
...
export const getAchievementTypes = (
  userId: any,
  perspective: "global" | "personal" = "global"
) => ({
  day: {
    first: {
...
export const comparePersonalAndGroupScore = (
  personal: number,
  group: number
) => {
  return {
...
const discoveredAndFormatAchievements = (
  topArtistsByPeriod: TopArtistsByPeriodType,
  userId: string,
  period: AchievementPeriodType
) => {
  return topArtistsByPeriod[period]
...
const determinePossibleAchievements = (
  achievements: any = {},
  topArtistsByPeriod: TopArtistsByPeriodType,
  userId: string
) => {
  ...
  for (let period in topArtistsByPeriod) {
    const derived: any = discoveredAndFormatAchievements(
      topArtistsByPeriod,
      userId,
      period as AchievementPeriodType
    );
...
export const parseAchievementsByPeriod = (
  usersTopArtistByPeriodData: any,
  userId: string
) => {
  console.log(
    "TCL: parseAchievementsByPeriod -> usersTopArtistByPeriodData",
  ...
  const { existingAchievements }: any = determinePossibleAchievements(
    achievements,
    topArtistsByPeriod,
    userId
  );
...
export const getAchievementHistory = (userId: any) => {
  ...
  const {
    getUserAchievements: weeklyAchievements
  }: any = weeklyAchievementData;
