...
export const Artists: React.SFC<RouteComponentProps & {
  uid: string;
  pathParams: TPathParams;
}> = ({ uid, pathParams }) => {
  const {
    insightsArtists: {
      [pathParams.timeScope]: { [pathParams.perspective]: artists }
    }
  } = suspensefulHook(
    useInsightsArtists({ variables: { uid }, suspend: true })
  );
  ...
  return (
    <>
      <h1 style={{ color: "#fff", margin: "32px 0" }}>
        <InsightsBackLink /> {translatedPerspective} Top Artists{" "}
        {normalizetimeScopeMap[timeScope]}{" "}
      </h1>
